...
- **Coach earnings & payouts (Feb 2026)**: mirrors the ambassador flow but attribution is via `coaching_clients.coach_id` (not promo code) and the amount owed per order is the full order **total** (admin decides how much of the pending pool to actually pay out on each settlement). New sidebar entry `/admin/coach-earnings` (`AdminCoachEarnings.jsx`) — list of coaches with orders/gross/paid/pending; click a coach → modal with KPI grid, orders table with tick-to-select payout builder, payout recording and payout history (delete reverts flags). Coach self-view at `/coach/earnings` (`CoachEarnings.jsx`) — read-only KPI + attributed orders + payout history. Backend: `coach_earnings_routes.py` (`GET/POST/DELETE` under `/coaching/admin/coach-earnings/*` and `GET /coaching/coach/earnings`). New collection `db.coach_payouts`. Orders get `coach_payout_paid`, `coach_payout_id`, `coach_payout_paid_at`, `coach_user_id` fields on settlement.
