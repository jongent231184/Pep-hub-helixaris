...
- **WhatsApp community CTA (Feb 2026)**: admin-editable community-invite block on the paid order-confirmation email + `/order-confirmation` thank-you page. Three new Settings fields (`whatsapp_invite_url` + headline + body), rendered conditionally so the block disappears when the URL is blank. Green "Join the group" button opens the configured URL in a new tab. Works for any invite (Telegram/Discord too — just paste the URL).
- **Peptide calculator v2 (Feb 2026)**: two upgrades to reconstitution UX. **mcg / mg toggle** on Step 4 that preserves the effective dose when switching (no accidental 1000× scaling); results panel shows both units side-by-side. **Visual insulin syringe SVG** in the results panel showing the exact draw position — barrel with graduation ticks matching the selected syringe (30u/50u/100u), emerald fill from needle end to calculated stop, sky-blue marker line + units label. Amber fill when overflow.
- **Cross-repo mirror workflow (Feb 2026)**: Emergent's one-repo-per-app rule created a mirror repo `Pep-hub-helixaris` (public) as an exact clone of `Pep-hub` (private, primary). This pod pushes to both — primary via Emergent's Save-to-GitHub, mirror via `/app/scripts/mirror-to-helixaris.sh` (fine-grained PAT stored in gitignored `.mirror_token`). Helixaris pod pulls updates via `git remote add origin https://github.com/jongent231184/Pep-hub-helixaris.git && git fetch origin main && git reset --hard origin/main`. Single-chat workflow preserved.
- **Square Payment Links integration LIVE (Feb 2026)** — full card acquiring rail alongside Wallid open-banking. Customer-facing "Pay by Card · Apple Pay · Google Pay" button on checkout below the Pay by Bank button, gated by the compliance tickbox. Server-side flow: `POST /api/square/create-checkout` generates a hosted paylink with a **single generic line item "GHP Order #N"** — peptide product names never touch Square's transaction descriptors (Path 1 hybrid). Amount + line-item detail live entirely in our DB; customer still sees the full itemised invoice in email + on-site. Webhook flow: `POST /api/square/webhook` with inline HMAC-SHA256 signature verification supporting **multiple signing keys + multiple URLs** simultaneously (one codebase serves preview + live subscriptions), replay defence via unique `event_id` index, amount tamper defence on the DB update. Safety-net poller `POST /api/square/reconcile` called from the order-confirmation page — if the webhook hasn't fired within a few seconds of redirect, we poll Square directly and mark paid. Admin sync UI: bulk "Sync from Square" on `/admin/orders` and per-order "Sync from Square" on `/admin/orders/:id` — mirror of the Wallid sync affordances. SDK: `squareup==46.0.0.20260916`. Env-var driven, sandbox/production toggle in one line. **In production as of Feb 2026** — real cards accepted at www.ghp-health.com with dual webhook subscriptions (preview + live).
- **Future: Xoalia merchant swap** — Xoalia UK high-risk card acquiring is currently in underwriting review (aware of research peptides, no ToS ambiguity like Square). When approved, swap Square out and Xoalia in as the primary card rail. Because payment provider is driven entirely by admin env vars + a config-probe endpoint, the swap will be an env-var change + a new create-checkout / webhook implementation — no frontend rewrite required. Wallid stays as the parallel Pay-by-Bank rail.
